...
const cloudinaryImage = (id: string) => `https://res.cloudinary.com/dofv1fbu6/image/upload/${id}.jpg`;
const ImageBackdrop = ({ src, alt, children, className = '' }: { src: string; alt: string; children: React.ReactNode; className?: string }) => <div className={`imageBackdrop ${className}`}><img src={src} alt={alt}/><div className="imageBackdropContent">{children}</div></div>;
...
function Visual({ kind }: { kind: string }) {
  if (kind === 'cover') return <ImageBackdrop className="investorCover" src={cloudinaryImage('04e5becb141a8eb022938a4b98f47036_pz3hg8')} alt="Campo tático de futebol"><div className="cover-art"><span className="ghost">02</span><div className="cover-stack"><i/><i/><b>CAPITAL</b></div><p>CARREIRA<br/>+<br/>PATRIMÔNIO</p></div></ImageBackdrop>
  ...
  if (kind === 'vehicle') return <ImageBackdrop className="investorVehicle" src={cloudinaryImage('48e2f09e82354688785783390a013af8_u4cex2')} alt="Campo de futebol visto de cima"><div className="vehicle"><div className="vehicle-people"><span>INVESTIDORES</span><span>APORTES</span></div><div className="vehicle-core"><Building2/><strong>CNPJ<br/>TITULAR</strong><small>patrimônio imobiliário</small></div><div className="vehicle-tail"><span>AQUISIÇÃO</span><span>GOVERNANÇA</span></div></div></ImageBackdrop>
  if (kind === 'roles') return <ImageBackdrop className="investorRoles" src={cloudinaryImage('2f85d4289c89fb96de5f6f26446bd95c_pssz4e')} alt="Executivo em silhueta"><div className="roles"><div><Landmark/><small>TITULAR</small><strong>CNPJ</strong><span>propriedade<br/>do imóvel</span></div><div className="role-link">condições</div><div><UserRound/><small>BENEFICIÁRIO</small><strong>ATLETA</strong><span>direito futuro<br/>condicionado</span></div></div></ImageBackdrop>
  ...
  return <ImageBackdrop className="investorClosing" src={cloudinaryImage('89f8d5ccef552a38685f64a8586634ee_custd9')} alt="Microfones em uma coletiva"><div className="closing-mark"><div><span>C</span><span>I</span></div><p>CAPITAL<br/><b>INTELIGENTE</b></p><small>LUCAS SILVESTRE — 2026</small></div></ImageBackdrop>
}
