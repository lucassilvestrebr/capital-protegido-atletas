...
const PlainVisual = ({ children, className = '' }: { children: React.ReactNode; className?: string }) => <div className={`plainVisual ${className}`}>{children}</div>;
...
function Visual({ kind }: { kind: string }) {
  if (kind === 'cover') return <PlainVisual className="investorCover"><div className="cover-art"><span className="ghost">02</span><div className="cover-stack"><i/><i/><b>CAPITAL</b></div><p>CARREIRA<br/>+<br/>PATRIMÔNIO</p></div></PlainVisual>
  ...
  if (kind === 'vehicle') return <PlainVisual className="investorVehicle"><div className="vehicle"><div className="vehicle-people"><span>INVESTIDORES</span><span>APORTES</span></div><div className="vehicle-core"><Building2/><strong>CNPJ<br/>TITULAR</strong><small>patrimônio imobiliário</small></div><div className="vehicle-tail"><span>AQUISIÇÃO</span><span>GOVERNANÇA</span></div></div></PlainVisual>
  if (kind === 'roles') return <PlainVisual className="investorRoles"><div className="roles"><div><Landmark/><small>TITULAR</small><strong>CNPJ</strong><span>propriedade<br/>do imóvel</span></div><div className="role-link">condições</div><div><UserRound/><small>BENEFICIÁRIO</small><strong>ATLETA</strong><span>direito futuro<br/>condicionado</span></div></div></PlainVisual>
  ...
  return <PlainVisual className="investorClosing"><div className="closing-mark"><div><span>C</span><span>I</span></div><p>CAPITAL<br/><b>INTELIGENTE</b></p><small>LUCAS SILVESTRE — 2026</small></div></PlainVisual>
}
